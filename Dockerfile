...
RUN R -e "source('https://bioconductor.org/biocLite.R'); biocLite('circlize'); biocLite('edgeR'); biocLite('DESeq2');"
RUN R -e "install.packages(c('shinyjs', 'shinyBS', 'RMySQL', 'shinycssloaders'), repos = 'http://cran.us.r-project.org');"

RUN R -e "install.packages(c('devtools', 'rsconnect', 'httr'), repos = 'http://cran.us.r-project.org');"
...
RUN R -e "install.packages(c('locfit'), repos = 'http://cran.us.r-project.org');"
